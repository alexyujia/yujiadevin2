...
export module libra.kernel.tool.wwise.sound_bank.decode;
	...
	struct Decode :
		...
		inline static auto convert_common_property_as_randomizable (
			...
			if (auto element_if = map.randomizer.query_if(type)) {
				...
				value.minimum_value = mbox<Value>(0);
				value.maximum_value = mbox<Value>(0);
			}
			return;
		}
		...
		inline static auto exchange_id (
			...
			data.read(raw_value);
			value = cbox<typename Definition::ID>(raw_value);
			return;
		}
