...
$env:ANDROID_NDK_ROOT="D:\Android_SDK\ndk\android-ndk-r28b"
...
if ($TargetPlatform -eq "windows.x86_64") {
	...
	xmake "build"
	Copy-Item -Force -Recurse -Path "${ModuleDirectory}/.build/windows/x64/release/yujia.dll" -Destination "${ModuleDistributionFile}"
}
if ($TargetPlatform -eq "linux.x86_64") {
	...
	xmake "build"
	Copy-Item -Force -Recurse -Path "${ModuleDirectory}/.build/linux/x86_64/release/libyujia.so" -Destination "${ModuleDistributionFile}"
}
if ($TargetPlatform -eq "macintosh.x86_64") {
	...
	xmake "build"
	Copy-Item -Force -Recurse -Path "${ModuleDirectory}/.build/macosx/x86_64/release/libyujia.dylib" -Destination "${ModuleDistributionFile}"
}
if ($TargetPlatform -eq "android.arm_64") {
	...
	xmake "build"
	Copy-Item -Force -Recurse -Path "${ModuleDirectory}/.build/android/arm64-v8a/release/libyujia.so" -Destination "${ModuleDistributionFile}"
}
if ($TargetPlatform -eq "iphone.arm_64") {
	...
	xmake "build"
	Copy-Item -Force -Recurse -Path "${ModuleDirectory}/.build/iphoneos/arm64/release/libyujia.dylib" -Destination "${ModuleDistributionFile}"
}
Pop-Location
